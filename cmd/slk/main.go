...
func run(startupLink *slackurl.Permalink) error {
	...
	proto, pxW, pxH := resolveImageCapabilities(cfg.Appearance.ImageProtocol)
	...
	// terminal reports no usable geometry. pxW/pxH are the measured
	// metrics from resolveImageCapabilities above.
	...
	for _, ot := range orderedTokens {
		go func(tok slackclient.Token) {
			...
			wctx, err := connectWorkspace(ctx, tok, db, cfgSnap, avatarCache, p, configPath, paneRestore)
			if err != nil {
