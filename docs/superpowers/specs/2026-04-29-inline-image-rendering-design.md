...
- **Cell-metrics fallback `(8, 16)`** may produce slightly off-target downscales on hi-dpi terminals that answer none of `$COLORTERM_CELL_*`, `TIOCGWINSZ`, or the XTWINOPS `CSI 16t` startup probe (the last covers ptys that strip pixel dimensions, e.g. docker). Terminals that do answer are measured, and both pixel-addressed renderers (sixel and kitty) encode against the measured size, so the thumb we pick is resampled to the cell box's real device pixels rather than an assumed 8×16.
