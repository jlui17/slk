...
func TestNormalModeKeys(t *testing.T) {
	...
	runKeyCases(t, ModeNormal, []keyCase{
		...
		// -------------------------------------------------------------
		{
			// Fork: `O` is OpenLinkTab (keys.go), so preview is `v` only.
			name:     "v emits OpenImagePreviewMsg for the first image attachment",
			opts:     normalOpts(),
			setup:    focusMessages,
			key:      keyPress('v'),
			wantMode: ModeNormal,
		...
		{
			name:     "v on a message with no image attachment is a silent no-op",
			opts:     normalOpts(),
			setup:    func(t *testing.T, a *App) { focusMessageAt(t, a, 0) },
			key:      keyPress('v'),
			wantMode: ModeNormal,
